...
// 照片放在 public/images/gallery/，命名 gallery-01.webp ~ gallery-62.webp
// （01~33 是第一批、34~50 是新人 2026-09-15 自己修的第二批，長邊 1566；36 與 40 同場景重複，新人拿掉 36，編號留空不補位）。
// 51~62 是 2026-09-17 的第三批（~/Downloads/0917已修，12 張、長邊 1567），編號＝LINE 相簿序號＋50：
// 其中 _6、_9、_10 是 06、42、50 同一格畫面的重修版，直接蓋掉原檔、不另給編號，所以 56、59、60 留空；實際新增 9 張。
// 分組依照片實際場景（草原粉紗／海邊白紗／都會黑禮服）判定；要調整只需改下方三個編號陣列。
// 橫式只有 01~09、37、43、45、48、52、55、57，其餘都是直式——選單縮圖、系列首圖這些 3:2 裁切的位置只能挑橫式。
...
const PHOTO_ALT: Record<number, string> = {
  ...
  33: '海邊夕陽下頭紗飛揚，新人親吻臉頰',
  // 第二批（34~50）
  34: '都會石造建築前的獨照，黑色印花蓬裙與大蝴蝶結',
  35: '新人倚著石牆親暱相望，手持紅玫瑰，黑色印花禮服',
  37: '林間樹下相擁，彩色光斑飄滿畫面，粉紗與灰西裝',
  38: '草坡上笑著朝鏡頭伸手的獨照，粉色禮服',
  39: '新郎在前景舉相機，新娘在草坡上笑得掩嘴，粉紗',
  40: '精品店門前捧著黑色愛心氣球相視而笑，黑禮服與酒紅西裝',
  41: '樹蔭下親吻，彩色圓形光斑環繞，粉紗長裙',
  42: '海邊夕陽逆光下親吻，共持一束捧花',
  43: '沙灘上頭紗迎風揚起的獨照，夕陽映在海面',
  44: '牽手走在沙灘上回眸，白紗頭紗與淡紫捧花',
  45: '新人走在山谷長草間，遠山與綠地環繞',
  46: '草地上新娘捧花坐著、新郎俯身依偎的近拍，粉紗',
  47: '溪畔草坡上並坐，新郎親吻新娘臉頰，粉紗鋪滿草地',
  48: '林蔭下草坪上的全身合影，粉紗與灰西裝',
  49: '木橋欄杆邊由身後環抱，粉紗與森林綠意',
  50: '沙灘上單手揚起長頭紗的獨照，夕陽金光',
  // 第三批（51~62，56／59／60 留空）
  51: '林蔭步道旁的草地上並肩站立，粉紗長拖尾鋪在草地上',
  52: '山林木橋上牽手走來，藍天與綠樹環繞，粉紗與灰西裝',
  53: '長草坡上新郎蹲在新娘身旁，兩人拿著拍立得相機對鏡頭笑',
  54: '海邊沙丘上由身後環抱，逆光下白紗長拖尾鋪展',
  55: '新郎單手拿著捧花站在海邊草丘上遠望的獨照，灰西裝',
  57: '夕陽海邊新郎從身後親吻新娘臉頰，新娘捧花笑開',
  58: '夕陽逆光下新郎從身後環抱親吻，新娘捧著紫色捧花大笑',
  61: '沙灘上頭紗隨風揚起的獨照，手捧淡色捧花回望',
  62: '夕陽沙灘上新郎蹲在新娘身旁輕撫她的臉，白紗鋪在沙上',
}
...
/** 橫式照片的編號（其餘都是直式） */
const LANDSCAPE = new Set([1, 2, 3, 4, 5, 6, 7, 8, 9, 37, 43, 45, 48, 52, 55, 57])
/**
 * 不是該批標準尺寸的幾張（實際量的 webp 尺寸）：第二批新人裁過的 34、44；
 *  第三批重修版蓋掉的 06、42、50（尺寸跟著新檔走，不再是原本那一批的標準）與裁過的 52、61
 */
 ...
 * 第一批長邊 1600（1600×1111／1111×1600），第二批長邊 1566（1566×1044／1044×1566），第三批長邊 1567（1567×1045／1045×1567）
 */
function photoSize(num: number): [number, number] {
  const override = SIZE_OVERRIDE[num]
  if (override)
    return override
  const [long, short] = num <= 33 ? [1600, 1111] : num <= 50 ? [1566, 1044] : [1567, 1045]
...
interface SeriesSeed {
  ...
  wordSplit: number
  description: string
  /** 系列頁首圖編號（滿版出血會裁掉上下，橫式優先——橫式只有 01~09、37、43、45、48、52、55、57） */
  cover: number
  /** 首圖的垂直裁切錨點；直式照片要把這個值調到人臉的高度，否則寬版裁切會切到頭 */
  coverFocus: string
  /** 頁尾「下一個系列」預告卡編號（非直視鏡頭，橫式優先——橫式只有 01~09、37、43、45、48、52、55、57） */
  preview: number
  /**
   * PublicMenu 目錄列的縮圖編號。那裡是 3:2 置中裁切，直式會切掉頭或半身，
   * 所以只准填橫式（01~09、37、43、45、48、52、55、57）；city 系列到第二批仍沒有橫式照片，只能借別組的一張。
   */
  menuThumb: number
...
const SERIES_SEEDS: SeriesSeed[] = [
  {
    ...
    artwork: 'meadow.webp',
    windmills: true,
    // 第二、三批穿插進第一批之間（不是接在尾巴），系列頁往下捲三批才會交錯出現；showcase 也換進第二、三批，且不再用 hero 那張（01）。
    // 第三批（09-17）：53 換掉 23、51 換掉 25（showcase 的格子全是直式，橫式的 52 只進系列頁）；
    // 52 跟 02 都是木橋，系列頁裡隔開放
    showcase: [46, 53, 41, 38, 51, 49, 21],
    nums: [1, 2, 45, 3, 53, 4, 41, 5, 37, 52, 15, 16, 46, 17, 18, 47, 19, 51, 20, 38, 21, 22, 49, 23, 24, 48, 25, 39],
  },
  {
    slug: 'seaside',
    title: 'By the Ocean',
    ...
    bubbles: true,
    // 第三批（09-17）：showcase 換進 54、58、62、61（換掉 28、31、09、50）——格子全是直式，原本那張橫式的 09 會被裁成直條；
    // 手機只演前四張，前四張就有兩張新的。57 與 58 是同一刻的橫直兩版，57 只進系列頁、兩張隔開放；
    // 55（新郎獨照）不跟 27（也是新郎獨照）相鄰
    showcase: [54, 42, 58, 44, 62, 61, 29],
    nums: [6, 7, 57, 43, 8, 54, 9, 55, 26, 61, 44, 58, 27, 28, 42, 62, 29, 30, 31, 50, 32, 33],
  },
  {
    slug: 'city',
    title: 'In the City',
    ...
    menuThumb: 4,
    artwork: 'city.webp',
    // 14 是這一系列的 cover，不再同時放進 showcase
    showcase: [35, 10, 13, 34, 11],
    nums: [10, 34, 11, 12, 35, 13, 14, 40],
  },
]
...
export function useGalleryContent(): GalleryContent {
  ...
  return {
    ...
    // 三個主題混排、直橫交錯，最後由 hero 收尾；五張都不跟三個系列的 showcase 重複
    montage: [3, 40, 47, 43, 26].map(photoSrc),
