...
## Background
- 新人規劃六類禮物：桌上禮（table）、二進禮（second_entrance）、遊戲禮（game）、送客禮（send_off）、探房禮（room_visit）、喝茶禮（tea_ceremony）
...
1. 管理員能在六個固定類別下新增禮物品項，品項落在正確類別區塊
...
- 全部總額 18,350
- 端點：GET/POST `.../gift-items`、PATCH/DELETE `.../gift-items/{giftItemId}`；POST 缺 description → 400「請輸入款式說明」
