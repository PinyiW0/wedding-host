...
import { defaultRsvpFormConfig, normalizeRsvpBanners, normalizeRsvpQuestions } from '../../../../mock/data/rsvp-config'
...
export default defineEventHandler(async (event: H3Event): Promise<RsvpFormConfigDetail> => {
  ...
  return {
    ...
    // 多組欄位是後來才加的：舊存檔只有單張 banner 時包成一組一張
    banners: normalizeRsvpBanners(config.banners, config.banner),
