...
- **緩解（issue #170）**：後台複製的三種婚禮層級公開連結（公開回覆、故事、喜帖）改發 `/s/<code>` 短網址，`w.` 簽名不再出現在新人發出去的網址上——賓客截圖或轉傳時不會連簽名一起外流。簽名於 `server/routes/s/[code].get.ts` 轉址時現算，`short_links` 表只存「短碼 → 婚禮＋連結種類」，所以密鑰輪換不會讓已發出的短連結失效（短碼本身不帶授權）。
